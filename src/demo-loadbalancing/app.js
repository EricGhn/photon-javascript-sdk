    ...
    DemoLoadBalancing.prototype.setupUI = function () {
        ...
        btnJoinOrCreate.onclick = function (ev) {
            if (_this.isInLobby()) {
                ...
                _this.joinRoom(gameId.value.length > 0 ? gameId.value : undefined, { createIfNotExists: true, expectedUsers: expectedUsers.value.length > 0 ? expectedUsers.value.split(",") : undefined });
            }
            else {
                _this.output("Reload page to connect to Master");
            }
        ...
        btnNew.onclick = function (ev) {
            if (_this.isInLobby()) {
                ...
                var expectedUsers = document.getElementById("expectedusers");
                _this.createRoom(name.value.length > 0 ? name.value : undefined, { expectedUsers: expectedUsers.value.length > 0 ? expectedUsers.value.split(",") : undefined, maxPlayers: 6 });
            }
            else {
                _this.output("Reload page to connect to Master");
            }
